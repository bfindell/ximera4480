...
To think about the whole function, imagine varying through all possible input values.  The whole function is not just the output values but rather all the correspondences between the input values and output values.  Thus, if the domain of $f$ is $D$, we may write $f=\{(x,y) | x\in D, y=f(x)\}$.  In other words, the function $f$ is this set of ordered pairs---which is why it is helpful to identify a function with its graph!
...
\begin{solution}
A function $I$ is called the identity function on domain $D$ if $f(I(x)) = \answer{f(x)}$ and $I(f(x))=\answer{f(x)}$ for all $x\in \answer{D}$ and for any function $f$ with domain $\answer{D}$.  
\end{solution}
\end{question}
